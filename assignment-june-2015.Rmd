...
`r nrow(test_orig)` rows and `r ncol(test_orig)` columns, and
...
`r ncol(train_data)` columns in both data frames.
...
select from these `r ncol(train_data) - 1` variables the ones that are
...
thus, those `r na_perc_df[2,2]` columns were also removed.
